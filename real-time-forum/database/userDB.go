...
func GetAllUsers(r *http.Request) []map[string]any {
	curr_user := GetCurrentUser(r)
	if curr_user == nil {
		return nil
	}
	rows, err := DB.Query("SELECT * FROM users WHERE id != ?", curr_user.ID)
	if err != nil {
	...
	for rows.Next() {
		...
		users = append(users, map[string]any{
			...
			"lastmessage": GetLastMessageWithUser(curr_user.Nickname, user.Nickname),
		})
	}
...
func GetLastMessageWithUser(senderID string, receiverID string) time.Time {
	query := `
	SELECT  created_at
	FROM messages
	...
	`
	var date time.Time
	row := DB.QueryRow(query, senderID, receiverID, receiverID, senderID)
	err := row.Scan(&date)
	if err != nil {
		return time.Time{}
	}
...
func GetMessages(sender string, receiver string, limit int, offset int) ([]*variables.Message, error) {
    ...
    LIMIT ? OFFSET ?;
    `
	Rows, err := DB.Query(GetData, sender, receiver, receiver, sender, limit, offset)
	if err != nil {
		log.Fatal(err)
	}
	for Rows.Next() {
