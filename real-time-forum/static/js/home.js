...
function formatUsers(users) {
  ...
  connected.sort((a, b) => {
    ...
    if (Date.parse(a.lastmessage) < 0 && Date.parse(b.lastmessage) < 0) {
      return a.user.nickname.localeCompare(b.user.nickname);
    }
    // Ensuite par ordre alphabétique pour les non connectés
    return Date.parse(b.lastmessage) - Date.parse(a.lastmessage);
  });
  not_connected.sort((a, b) => {
    // D'abord par statut de connexion (connectés en premier)
    if (Date.parse(a.lastmessage) < 0 && Date.parse(b.lastmessage) < 0) {
      return a.user.nickname.localeCompare(b.user.nickname);
    }
  ...
  for (let i = 0; i < connected.length; i++) {
    let user = connected[i].user;
    let isConnected = connected[i].connected;
  ...
  for (let i = 0; i < not_connected.length; i++) {
    let user = not_connected[i].user;
    let isConnected = user.connected;
